...
export class SimCardService {
  ...
  async create(payload: Partial<SimCard>) {
    ...
    console.log("Creating sim card with payload:", payload);
    const saved = await this.repo.save(ent);
    console.log("Sim card created with ID:", saved.id);
  ...
  async changeCustomer(id: number, customerId: number | null) {
    // Use an explicit QueryRunner here to demonstrate explicit rollback handling.
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    try {
      const simRepo = queryRunner.manager.getRepository<SimCard>(SimCard);
      const trxRepo =
        queryRunner.manager.getRepository<SimTransaction>(SimTransaction);
      const custRepo = queryRunner.manager.getRepository<Customer>(Customer);
      ...
      // commit and return the updated sim (loaded via the transaction manager)
      await queryRunner.commitTransaction();
      return queryRunner.manager.findOne(SimCard, {
        where: { id },
        relations: ["simType", "transactions", "transactions.customer"],
      });
    } catch (err) {
      ...
      try {
        ...
        // ignore rollback error but prefer to surface original error
      }
      throw err;
    } finally {
